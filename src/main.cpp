...
#if defined(NDEBUG)
# error "Westlandcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x470f547ec40de01d199eccfc72eccdb28f27cdda1d7bd5635520c8357613dc3e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Westlandcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Westlandcoin Signed Message:\n";
...
{
    // Westlandcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Westlandcoin: 10k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1.5 * 24 * 60 * 60; // Westlandcoin: 1.5 days
static const int64 nTargetSpacing = 10 * 60; // Westlandcoin: 10 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Westlandcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Westlandcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3396c592aa3dde46f64b221e2563674416df65a6ef47cb9a294e3c69b2d65fb3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048396ef8f48559dc11d80a15b7a8b5007eab1cfff83494cc324814378372f1ac76bbfc46673856f0cd8b5ebe0096ba71e7e1769525710a81da83169a7bb69b9e7") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1520357620;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085191577;

        if (fTestNet)
        {
            block.nTime    = 1520357540;
            block.nNonce   = 388404044;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x2eb5adcae91c59e45ca1b7da2e4387c697aad4dc83f72c622f652320e3febea4"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc2, 0xbc, 0xd2 }; // Westlandcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WestlandcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WestlandcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WestlandcoinMiner(CWallet *pwallet)
{
    printf("WestlandcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("westlandcoin-miner");
    ...
    try { loop {
        ...
        printf("Running WestlandcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("WestlandcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WestlandcoinMiner, pwallet));
}
